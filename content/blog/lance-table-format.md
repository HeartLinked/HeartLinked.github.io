...
- 倒序版本号
  ...
  版本越新，数字越小，字典序越靠前。LIST `_versions/` O(1) 取第一条，就是最新版本。

  > 注意字典序和数值序不一样。字符串比较里 "10" < "9"，所以文件名必须定长补零到 20 位。另外 Lance 早期版本就是 `1.manifest`、`2.manifest` 这样命名的（Naming Scheme V1），后来才换成翻转补零的 V2 方案，老表还能读，新建表默认用 V2。
...
- `latest_version_hint.json` 加速
  ...
  - 先 GET 一个很小的 `latest_version_hint.json`，里面写着比如 `{"version": 42}`。这代表"**至少**已经有 42 版了"——因为它可能过期（别人刚提交了 43，还没来得及更新 hint）。
...
- 可扩展的 catalog 服务：跨表治理
  ...
  - 多引擎共享：Spark、Trino、Ray、你的 Python 服务都要访问同一批表，需要一个统一的入口和一致的 schema 视图。
  - 跨表原子操作：同时把 A、B 两张表切到新版本，或者整库回滚。文件名方案只能保证单表原子。
  - 血缘、审计、生命周期管理这类治理需求。
...
1. HEAD 请求 → 拿到文件大小 size。
2. GET range `[size-N, size)` → 读 footer，解析出 `manifest_pos`。
3. GET range `[manifest_pos, size)` → 读正文。

实际上，Lance 读版本时会先 LIST `_versions/` 取第一条，而 LIST 返回的每个条目本来就自带 key、size、last-modified 等信息。既然大小已经到手，第 1 步的 HEAD 就可以直接跳过。之后 Lance 不是只读 footer 那几十个字节，而是**直接读最后 64 KiB** `[size - 64KiB, size)`。绝大多数表的 manifest 都不到 64 KiB，所以这一次 range 读就把 footer 和整个正文都拿到了，从而流程变成：

1. LIST `_versions/` 取第一条。
2. GET `[size - 64KiB, size)`。
3. 若还有剩余部分，切成若干块并发 GET range，总耗时 1 次往返。

具体来说，File Footer 结构基本如下，包含正文段的 offset、版本、Magic Code 3 个部分：

```
[正文若干段 ...]
[几个 "某某段从第几字节开始" 的偏移]
[u16 major | u16 minor]   ← 格式版本
...
- **IndexSection**（可选）：这张表所有索引的元数据。写在最前面，Manifest 里用 `index_section` 字段记它的 Offset。没有索引就整段不写。
...
每个 `Field` 有：`id`、`name`、`logical_type`、`nullable`、`metadata`（三类：`lance-encoding:*` 编码配置、`lance-schema:unenforced-primary-key` 不强制主键、`ARROW:extension:*`）、以及主键/聚簇键位置。

field id 的规则和 Iceberg 完全一致：建表时按深度优先从 0 分配；之后新增字段拿下一个可用 id；一旦分配永不改变、永不复用。
...
- `deletion_file`：可选，只是一个**引用**——`file_type`（ARROW_ARRAY / BITMAP）、`read_version`、`id`、`num_deleted_rows`——内容在 `_deletions/{fragment_id}-{read_version}-{id}.arrow|.bin`。每个 fragment 在一个版本里最多一个删除文件，再删就写一个合并后的新文件替换引用。
- `physical_rows`：含已删行的总行数。逻辑行数 = `physical_rows - deletion_file.num_deleted_rows`。
- `row_id_sequence`、`created_at_version_sequence`、`last_updated_at_version_sequence`：开启稳定行号后才有，小于约 200 KB 内联为 bytes，否则写外部文件用 `ExternalFile{path, offset, size}` 引用。
- `overlays[]`：数据覆盖文件，实验特性，略。
